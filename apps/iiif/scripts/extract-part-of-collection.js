...
export const extractPartOfCollection = {
  ...
  async collect(temp, api, config) {
    ...
    for (const [slug, mapping] of items) {
      if (mapping.type === "Collection") {
        ...
        for (let [collectionId, manifestIds] of Object.entries(
          collectionItems,
        )) {
          ...
          for (const manifestId of manifestIds) {
            ...
            manifestIsInCollection[manifestSlug] =
              manifestIsInCollection[manifestSlug] || [];
...
extract(extractPartOfCollection, extractPartOfCollection.handler);
