...
extract(
  ...
  async (resource, api, config) => {
    ...
    const plaintext = api.resource.metadata
      .map((i) => getValue(i.value))
      .join(" ");

    // Exclude illustrations from index
    if (collectionSlugs.includes("illustrations")) {
      return {}
    }
    ...
    return {
      search: {
        ...
        record: {
          ...
          // collections: collections.map((c) => getValue(c.label)),
        },
      },
    };
  },
